...
/*   Updated: 2025/03/13 16:57:08 by urlooved         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*t_handler_philo(void *data)
{
	...
	if (philo->table->amount_phis == 1)							// 1 philo process
	...
	else if (philo->table->amount_phis % 2 != 0 && philo->id == philo->table->amount_phis - 1)	// Only for ODD amount of philos. The last one will think by default 
		think_process(philo);
